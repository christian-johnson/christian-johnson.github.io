...
const csvPath = process.argv.find(
  (arg) =>
    !arg.startsWith("--") &&
    !arg.includes("node") &&
    !arg.includes("seed-firebase-views"),
);
if (!csvPath) {
  ...
  console.error(
    "  --force      Overwrite existing documents (useful for re-importing)",
  );
  process.exit(1);
}
...
async function importToFirestore(data) {
  ...
  for (let i = 0; i < data.length; i += batchSize) {
    ...
    for (const item of chunk) {
      ...
      if (existing.exists) {
        if (FORCE) {
          ...
          console.log(
            `  Skipping ${item.docId} (already exists with ${existing.data().views} views)`,
          );
          skipped++;
          continue;
        }
    ...
    console.log(
      `  Processed ${Math.min(i + batchSize, data.length)}/${data.length}`,
    );
  }
...
async function main() {
  ...
  if (data.length === 0) {
    ...
    console.error(
      'Expected columns: "Page path" (or similar) and "Views" (or similar)',
    );
    process.exit(1);
  }
